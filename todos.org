...
*** TODO When loading the extension for the first time, the loading spinner does not
    display (but it should) for while the extension is adding the necessary labels
    and/or loading initial emails, etc.
*** TODO When loading the extension in an email inbox that has less than 5 email threads,
